...
function Pedidos({ productos, clientes, pedidos, currentUser }) {
  ...
  const crearPedido = async estado => {
    ...
    if (estado === 'asignado_pendiente_transferencia' && !repartidorId) { flash('⚠️ Elige al repartidor responsable antes de asignar'); return; }
    setSaving(true);
    try {
      const fecha = new Date().toISOString();
  ...
  return React.createElement('div', { style: { padding: '16px 12px' } },
    ...
    React.createElement(Card, null,
      ...
      !puedeAsignar && React.createElement('div', { style: { fontSize: 11, color: 'var(--ink-faint)', marginBottom: 10 } }, 'El pedido se guardará como borrador para que administración asigne al repartidor.'),
      React.createElement(Row, { style: { gap: 8 } }, React.createElement(BOut, { onClick: () => crearPedido('borrador'), disabled: saving, style: { flex: 1 } }, 'Guardar borrador'), puedeAsignar && React.createElement(BFill, { onClick: () => crearPedido('asignado_pendiente_transferencia'), disabled: saving, style: { flex: 1 } }, saving ? 'Guardando…' : 'Asignar pedido'))
    ),

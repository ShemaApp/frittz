...
function Pedidos({ productos, clientes, pedidos, currentUser }) {
  ...
  const updQty = (id, cant) => setCart(actual => actual.map(x => {
    if (x.id !== id) return x;
    if (cant === '') return { ...x, cant: '' };
    const n = Number(cant);
    return Number.isFinite(n) ? { ...x, cant: Math.max(0, n) } : x;
  }));
  const blurQty = id => setCart(actual => actual.filter(x => x.id !== id || Number(x.cant) > 0));
  ...
  return React.createElement('div', { style: { padding: '16px 12px' } },
    ...
    React.createElement(Card, null,
      ...
      cart.length > 0 && React.createElement('div', { style: { margin: '10px 0', borderTop: '1px solid var(--line)', paddingTop: 8 } }, cart.map(item => React.createElement(Row, { key: item.id, style: { gap: 6, justifyContent: 'space-between', marginBottom: 7 } }, React.createElement('span', { style: { flex: 1, fontSize: 12, fontWeight: 600 } }, item.nombre), React.createElement('input', { type: 'number', min: 1, value: item.cant === '' ? '' : item.cant, onChange: e => updQty(item.id, e.target.value), onBlur: () => blurQty(item.id), style: { width: 48, textAlign: 'center', padding: 4, border: '1px solid var(--line-strong)', borderRadius: 4, background: 'var(--surface-2)', color: 'var(--ink)' } }), React.createElement('span', { style: { minWidth: 60, textAlign: 'right', fontSize: 12 } }, fmt(item.precio * (Number(item.cant) || 0)))), React.createElement(Row, { style: { justifyContent: 'space-between', borderTop: '1px solid var(--line)', paddingTop: 8, marginTop: 4 } }, React.createElement('strong', null, 'Total previsto'), React.createElement('strong', { style: { color: 'var(--accent-text)' } }, fmt(total)))),
